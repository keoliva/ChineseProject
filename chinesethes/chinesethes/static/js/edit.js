$(function() {
	...
	$('input#queryFieldInput').keyup(function(e) {
		...
		if ($(this).val() == "") {
			...
			filter('li.list-group-item', 
				['div input[name="chinese"]', 'div input[name="pinyin"]', 'div textarea[name="english"]'], 
				$(this).val());
		}
	...
	function wordInfo(word) {
		return word.chinese + " -- " + word.pinyin + " -- " + word.english;	
	}
	...
	$('#add_entry').submit(function(e) {
		...
		console.log("CHI: " + chinese + " PINYIN: " + pinyin + " ENG: " + english);
		$('#view_edit_entries').prepend(returnLi(chinese, pinyin, english));
